...
const AlarmList = () => {
  ...
  const onChangePage = async (obj) => {
    ...
    let data_ = await apiManager('alarms', 'list', obj);
    if (data_) {
      setData(data_);
    }
    setSearchObj(obj);
  ...
  const deleteItem = async (id) => {
    let data = await apiManager('alarms', 'delete', { id });
    if (data) {
...
AlarmList.getLayout = (page) => <ManagerLayout>{page}</ManagerLayout>;
